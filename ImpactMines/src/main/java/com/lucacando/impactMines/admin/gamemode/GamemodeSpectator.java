...
public class GamemodeSpectator implements CommandExecutor {
    ...
    public GamemodeSpectator(Main main, String usage) {
        this.main = main;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (main.isStaff(player)) {
                if (args.length < 1) {
                    player.sendMessage(main.prefix + "Gamemode set to spectator.");
                    player.setGameMode(GameMode.SPECTATOR);
                } else {
                    if (Bukkit.getPlayer(args[0]) != null) {
                        Player p2 = Bukkit.getPlayer(args[0]);
                        if (p2.isOnline()) {
                            player.sendMessage(main.prefix + ChatColor.YELLOW + p2.getDisplayName() + "'s " + ChatColor.BLUE + "gamemode set to spectator.");
                            p2.setGameMode(GameMode.SPECTATOR);
                            p2.sendMessage(main.prefix + "Your gamemode has been set to spectator.");
                        } else {
