...
public class GamemodeAdventure implements CommandExecutor {
    ...
    public GamemodeAdventure(Main main, String usage) {
        this.main = main;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (main.isStaff(player)) {
                if (args.length < 1) {
                    player.sendMessage(main.prefix + "Gamemode set to adventure.");
                    player.setGameMode(GameMode.ADVENTURE);
                } else {
                    if (Bukkit.getPlayer(args[0]) != null) {
                        Player p2 = Bukkit.getPlayer(args[0]);
                        if (p2.isOnline()) {
                            player.sendMessage(main.prefix + ChatColor.YELLOW + p2.getDisplayName() + "'s " + ChatColor.BLUE + "gamemode set to adventure.");
                            p2.setGameMode(GameMode.ADVENTURE);
                            p2.sendMessage(main.prefix + "Your gamemode has been set to adventure.");
                        } else {
